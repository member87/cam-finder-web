...
<template>
  <div class="overflow-x-auto relative m-2 rounded ml-2" v-if="store.info.cameras[0]">
    ...
    <div class="z-50 bg-zinc-900/90 fixed top-0 left-0 w-screen h-screen flex flex-col justify-center items-center" v-if="modalOpen">
      <div class="w-3/4 relative">
        <img :src="imgData" alt="No work :(" />
        <button @click="modalOpen = 0" class="bg-primary text-xl px-4 py-2 rounded shadow absolute right-0 m-2"><i class="fa-solid fa-xmark mr-1"></i> close</button>
      </div>
    </div>
