        ...
        try {
          this.markers.addLayer(
            L.marker([el["lat"], el["long"]], {
              ...
              `<a href="${route("view.camera", el.id)}">${el["ip"]}:${
                el["port"]
              }</a>`
            )
          );
  ...
  mounted() {
    ...
    this.markers = L.markerClusterGroup({
      iconCreateFunction: function (cluster) {
        return L.divIcon({
          html: `<div class="bg-gray-300 font-extrabold shadow w-8 h-8 rounded-full text-center flex items-center justify-center"><span>${cluster.getChildCount()}</span></div>`,
        });
      },
    });
